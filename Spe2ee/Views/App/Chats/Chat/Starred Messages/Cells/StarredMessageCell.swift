...
class StarredMessageCell: StarredMessageBaseCell {
    ...
    if viewModel.showDownloadButton {
      ...
      if viewModel.isDownloadComplete {
        ...
        case .photo:
          ...
          let newImage = UIImage.fromPath(viewModel.message.localFilename)
          filePreviewImageView.image = newImage
...
//          viewModel.getVideoThumbnail { [weak self] (image) in
//            guard let strongSelf = self, let image = image else { return }
//            strongSelf.filePreviewImageView.image = image
//          }
          
          if let img = viewModel.getVideoThumbnail() {
            filePreviewImageView.image = img
          } else {
            // retry in few milliseconds
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.4) { [weak self] in
              guard let strongSelf = self else { return }
              strongSelf.filePreviewImageView.image = strongSelf.viewModel.getVideoThumbnail()
            }
          }
...
extension StarredMessageCell: GalleryItemsDataSource {
