...
static const CGFloat kCaptionBarHeight = 100.0f;
...
- (void)initCaptionView {
  if (self.captionView == nil) {
    ...
    [self.sendBtn setImage:[UIImage imageNamed:@"paper_airplane"] forState:UIControlStateNormal];
...
- (void)viewDidLoad
{
  ...
  _navView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 80)];
  [_navView setBackgroundColor:[CLImageEditorTheme toolbarColor]];
  [self.view addSubview:_navView];
//  [_CLImageEditorViewController setConstraintsLeading:@(0) trailing:@(0) top:@(0) bottom:nil height:@(104) width:nil parent:self.view child:_navView peer:nil];
  ...
  [_navView addSubview:self.doneBtn];
...
- (void)viewWillLayoutSubviews {
  ...
  // layout the nav bar
  UIEdgeInsets safeArea = self.view.pinObjc.safeArea;
  [[[[[_navView.pinObjc top] left] right] height:104 + (safeArea.top / 2)] layout];
  [[[[[self.backBtn.pinObjc left:12] vCenter:safeArea.top > 0 ? 10 : 0] height:35] width:35] layout];
  [[[[[self.doneBtn.pinObjc right:12] vCenterTo:self.backBtn.edge.vCenter] height:35] width:35] layout];
  
  // Layout the Top Tools scrollview
  [[[[[[self.menuView.pinObjc vCenterTo:self.backBtn.edge.vCenter] rightOf:self.backBtn] marginLeft:10] leftOf:self.doneBtn] marginRight:10] layout];
  
  // Bottom captionView
  [[[[self.captionView.pinObjc bottom] left] right] layout];
  [[[[[self.sendBtn.pinObjc right:safeArea.right+20] top] height:60] width:60] layout];
  [[[[[[self.captionTextField.pinObjc leftOf:self.sendBtn] marginRight:10] left:safeArea.left+20] top:10] height:40] layout];
  [[[[[self.toLabel.pinObjc right:10] belowOf:self.captionTextField] left:10] height:30] layout];

  // Scroll View Image
  [[[[[[_scrollView.pinObjc left] right] belowOf: _navView] aboveOf: self.captionView] right] layout];
...
- (void)refreshToolSettings
{
  ...
  for(CLImageToolInfo *info in self.toolInfo.sortedSubtools){
    ...
    CLToolbarMenuItem *view = [CLImageEditorTheme menuItemWithFrame:CGRectMake(x+padding, 10, W, H) target:self action:@selector(tappedMenuView:) toolInfo:info];
