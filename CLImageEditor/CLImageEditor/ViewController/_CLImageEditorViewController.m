...
static const CGFloat kCaptionBarHeight = 110.0f;
...
//        menuScroll.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
        ...
        [_CLImageEditorViewController setConstraintsLeading:@(60) trailing:nil top:@(kNavBarHeight) bottom:nil height:@(menuScroll.height) width:@(self.view.width - 60 * 2) parent:self.view child:menuScroll peer:nil];
    }
}

- (void)initCaptionView {
    if (self.captionView == nil) {
        self.captionView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height - kCaptionBarHeight, self.view.width, kCaptionBarHeight)];
        self.captionView.backgroundColor = [CLImageEditorTheme toolbarColor];
        [self.view addSubview:self.captionView];
        
        
        
        self.captionTextField = [[UITextField alloc] init];
        [self.captionTextField setDelegate:self];
        UIView *leftView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 15, 40)];
        [leftView.layer setCornerRadius:20];
        leftView.backgroundColor = [UIColor whiteColor];
        self.captionTextField.leftView = leftView;
        self.captionTextField.leftViewMode = UITextFieldViewModeAlways;

        [self.captionTextField setPlaceholder:@"Add a caption..."];
        [self.captionTextField setBackgroundColor:[UIColor whiteColor]];
        [self.captionTextField setBorderStyle:UITextBorderStyleNone];
        [self.captionTextField.layer setCornerRadius:20];
        [self.captionView addSubview:self.captionTextField];
        [_CLImageEditorViewController setConstraintsLeading:@(20) trailing:nil top:@10 bottom:nil height:@40 width:@(self.captionView.width - 60 - 10) parent:self.captionView child:self.captionTextField peer:nil];

        
        
        self.sendBtn = [[UIButton alloc] init];
        [self.sendBtn setImage:[UIImage imageNamed:@"sendButton"] forState:UIControlStateNormal];
        [self.sendBtn addTarget:self action:@selector(tapSend:) forControlEvents:UIControlEventTouchUpInside];
        [self.captionView addSubview:self.sendBtn];
        [_CLImageEditorViewController setConstraintsLeading:nil trailing:@(10) top:@0 bottom:nil height:@(60) width:@(60) parent:self.captionView child:self.sendBtn peer:nil];
        
        self.toLabel = [[UILabel alloc] init];
        [self.toLabel setText:[NSString stringWithFormat:@"> %@", self.replyTo]];
        [self.toLabel setTextAlignment:NSTextAlignmentRight];
        [self.toLabel setTextColor:[UIColor grayColor]];
        [self.captionView addSubview:self.toLabel];
        [_CLImageEditorViewController setConstraintsLeading:nil trailing:@-10 top:nil bottom:@-30 height:@(30) width:@(self.captionView.width - 30) parent:self.captionView child:self.toLabel peer:nil];
    }
}
...
- (void)initImageScrollView
{
    if(_scrollView==nil){
        ...
        imageScroll.top = y;
        imageScroll.height = self.view.height - imageScroll.top - self.captionView.height;
...
//    [self initNavigationBar];
    ...
    UIView *navView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 104)];
    [navView setBackgroundColor:[CLImageEditorTheme toolbarColor]];
    [self.view addSubview:navView];
    [_CLImageEditorViewController setConstraintsLeading:@(0) trailing:@(0) top:@(0) bottom:nil height:@(104) width:nil parent:self.view child:navView peer:nil];
    
    self.backBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
    [navView addSubview:self.backBtn];
    [self.backBtn setImage:[UIImage imageNamed:@"closeButton"] forState:UIControlStateNormal];
    [self.backBtn addTarget:self action:@selector(tapBack:) forControlEvents:UIControlEventTouchUpInside];
    [_CLImageEditorViewController setConstraintsLeading:@(0) trailing:nil top:@(kNavBarHeight) bottom:nil height:@(60) width:@(60) parent:navView child:self.backBtn peer:nil];
    
    self.doneBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
    [navView addSubview:self.doneBtn];
    [self.doneBtn setImage:[UIImage imageNamed:@"doneButton"] forState:UIControlStateNormal];
    [self.doneBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.doneBtn addTarget:self action:@selector(tapDone:) forControlEvents:UIControlEventTouchUpInside];
    [_CLImageEditorViewController setConstraintsLeading:nil trailing:@(0) top:@(kNavBarHeight) bottom:nil height:@(60) width:@(60) parent:navView child:self.doneBtn peer:nil];
...
- (void)swapMenuViewWithEditing:(BOOL)editing
{
    [UIView animateWithDuration:kCLImageToolAnimationDuration
                     animations:^{
                         if(editing){
                             self->_menuView.transform = CGAffineTransformMakeTranslation(0, -self->_menuView.height - 44);
                         }
                         else{
                             self->_menuView.transform = CGAffineTransformIdentity;
                         }
...
//         ];
        [UIView animateWithDuration:kCLImageToolAnimationDuration animations:^{
            [self.backBtn setImage:[UIImage imageNamed:@"backButton"] forState:UIControlStateNormal];
...
- (void)tapDone:(UIButton*)sender {
    if (self.backBtn.selected) {
        ...
        if([self.delegate respondsToSelector:@selector(imageEditor:didSend:text:)]){
            [self.delegate imageEditor:self didSend:self->_originalImage text:self.captionTextField.text];
            [self dismissViewControllerAnimated:YES completion:nil];
        }
    }
}

- (void)tapOutside:(UITapGestureRecognizer*)tap {
    [self.captionTextField resignFirstResponder];
    self.activeTextfield = nil;
}

- (void)tapSend:(UITapGestureRecognizer*)tap {
    printf("send");
    if (self.currentTool == nil) {
        if(self.targetImageView==nil){
            if([self.delegate respondsToSelector:@selector(imageEditor:didSend:text:)]){
                [self.delegate imageEditor:self didSend:_originalImage text:self.captionTextField.text];
                [self dismissViewControllerAnimated:YES completion:nil];
            } else{
                [self dismissViewControllerAnimated:YES completion:nil];
            }
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        CGRect f = self.captionView.frame;
        f.origin.y -= keyboardSize.height;
        self.captionView.frame = f;
    }];
}
